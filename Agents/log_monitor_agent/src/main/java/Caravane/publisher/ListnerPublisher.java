...
/**
 * Legacy file watcher - DEPRECATED
 * Use LogTailerPublisher for log monitoring instead
 */
// @Component // Disabled - using LogTailerPublisher instead
    ...
    public void startwatching() {
        new Thread(() -> {
            try (WatchService watchService = FileSystems.getDefault().newWatchService()) {
                ...
                // this will define what kind of changes the watcher will listens to in our case
                // we the creation and the modification and the delition
                pathToWatch.register(watchService,
                        StandardWatchEventKinds.ENTRY_CREATE,
                        StandardWatchEventKinds.ENTRY_DELETE,
                        StandardWatchEventKinds.ENTRY_MODIFY);

                // now we will use something callsed the watchkey this objects will be the key
                // to access all the event that happened
                while (true) {
